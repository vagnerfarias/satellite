...
def return_matching_hg_id(hg_name):
    ...
    for hostgroup in hg['results']:
      ...
      if hostgroup['name'] == hg_name:
        hg_id = hostgroup['id']
	env_id = hostgroup['environment_id']
	return (hg_id,env_id)
...
def update_host_with_org():
	(myhgid,myenv) = return_matching_hg_id(HOSTGROUP)
	myhostid = return_matching_host_id(HOSTNAME)
	mylocid = return_matching_location(LOCATION)
	myorgid = return_matching_org(ORG)
	if VERBOSE:
	  print "------\nmyhgid: " + str(myhgid) + "\nmyhostid: " + str(myhostid) + "\nmylocid: " + str(mylocid) + "\nmyorgid: " + str(myorgid) + "\nmyenv: " + str(myenv) + "\n------"
	jsondata = json.loads('{"id": %s,"host": {"hostgroup_id": %s,"organization_id": %s,"location_id": %s, "environment_id": %s}}' % (myhostid,myhgid,myorgid,mylocid,myenv))
	...
	print_success("Successfully associated host with hostgroup, org, location & puppet environment")
